		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
			 	...
			 	$collapse = ''; if(isset( $value['collapse'] )) { $collapse = 'data-collapse="'.$value['id'].'"'; }
			 	$fold_id =''; if(isset($value['fold'])) { $fold_id = 'data-fold="'.$value['fold'].'"'; }
				...
				$visibility = $fold_vis = '';
				//if ( array_key_exists("collapse_sec",$value) ) {
					if (isset($value['collapse_sec'])) {
						$visibility="vs_".$value['collapse_sec']." temphide ";
					} 					
				//}
				
				//if ( array_key_exists("collapse_sec",$value) && array_key_exists("fold",$value) ) {
					if ( isset($value['collapse_sec']) && isset($value['fold']) ) {
						$fold_vis=" permhide ";
						
					} 					
				//}
	
				$output .= '<div id="section-'.$value['id'].'" '.$collapse.' ' .$fold_id. ' class="' . $visibility . $fold . $fold_vis . 'section section-'.$value['type'].' '. $class .'">'."\n";
