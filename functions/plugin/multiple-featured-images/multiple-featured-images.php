...
if ( ! class_exists( 'Cr_Featured_Image' ) ) {
	/**
	 * Featured image class.
	 */
	class Cr_Featured_Image {
		...
		public function meta_box_content() {
			...
			if ( $image_id ) {
				$preview_image = wp_get_attachment_image(
					$image_id, array( 266, 266 ), false, array(
						'class' => 'cr-preview-image',
					)
				);
				$remove_image_css = '';
			} else {
				$preview_image = '<img class="cr-preview-image" src="#">';
				$preview_image_css = '';
			}

			$preview_image = '<span class="cr-set-featured-image"' . $preview_image_css . '>' . $this->args['label_set'] . '</span>' . $preview_image;

			$set_image_link = '<p class="hide-if-no-js">';
				$set_image_link .= '<a aria-label="' . $this->args['label_set'] . '" href="#" id="' . $this->args['id'] . '" class="cr_upload_button">';
			...
			$remove_image_link = '<p class="hide-if-no-js cr-remove-featured-image"' . $remove_image_css . '>';
				$remove_image_link .= '<a aria-label="' . $this->args['label_remove'] . '" href="#" id="' . $this->args['id'] . '" class="cr-remove-image">' . $this->args['label_remove'] . '</a>';
			$remove_image_link .= '</p>';

			$nonce_field = wp_nonce_field( $this->args['nonce_action'], $this->args['nonce_name'], true, false );

			$output = '<div class="cr-featured-image-meta-box">' . $set_image_link . $remove_image_link . $nonce_field . '</div>';
...
if ( ! function_exists( 'cr_get_featured_image_id' ) ) {
	/**
	 ...
	 * @return int
	 */
	function cr_get_featured_image_id( $image_id, $post_type, $post_id = null ) {		
		if ( ! class_exists( 'Cr_Featured_Image' ) ) {
			return 0;
		}
		return Cr_Featured_Image::get_featured_image_id( $image_id, $post_type, $post_id );
	}
}
