...
/*-----------------------------------------------------------------------------------*/
	...
	function widget( $args, $instance ) {
		...
		$featured = ( $instance['featured'] == 'yes') ? 'yes' : 'no';
        ...
        <?php /* Display the widget title if one was input (before and after defined by themes). */
				...
				?>
                    ...
                    <?php if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post(); ?>
                    	<?php 
                    	if( $query->current_post == 0 && ($featured == 'yes') ) {
                    		?>
                    		<div class="featured_post">                   			
                                <div class="featured_post_thumb"><a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('blog-small'); ?></a></div>
                                <a href="<?php the_permalink(); ?>"><h2><?php the_title(); ?></h2></a>   
                                <span class="date"><?php the_time( get_option('date_format') ); ?></span>  
                                <div class="featured_post_excerpt"><?php echo string_limit_words(get_the_excerpt(), 18).'...'; ?></div>                         
                    		</div>
                    		<?php
                    	} 
                    	else {
...
/*-----------------------------------------------------------------------------------*/
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'featured' ) ); ?>"><?php esc_html_e( 'Make first post featured', 'Creativo' ); ?>:</label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'featured' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'featured' ) ); ?>" class="widefat">
				<option value="no" <?php selected( 'no', $featured ) ?>><?php esc_html_e( 'No', 'Creativo' ); ?></option>
				<option value="yes" <?php selected( 'yes', $featured ) ?>><?php esc_html_e( 'Yes', 'Creativo' ); ?></option>				
			</select>
		</p>
